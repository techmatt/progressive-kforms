...
{
	modelDatabase.init();
	curModel = modelDatabase.getRandomModel(app.graphics.castD3D11());
...
void Vizzer::render(ApplicationData &app)
{
	...
	assets.renderMesh(curModel->mesh, camera.getCameraPerspective());
...
void Vizzer::keyDown(ApplicationData &app, UINT key)
{
	...
	if (key == KEY_R) curModel = modelDatabase.getRandomModel(app.graphics.castD3D11());
}
...
void Vizzer::mouseMove(ApplicationData &app)
{
    ...
    const vec2i posDelta = app.input.mouse.pos - app.input.prevMouse.pos;
